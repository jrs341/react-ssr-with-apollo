  ...
  makeMinorYAxis(data) {
    ...
    for (var i = 1; i <= 14; i++) {
      ...
      if (i % 2 === 0){
        ...
        <text x={i * 5 + 3.1} y={svgHeight - 6} style={textStyle}>{hour < 12 ? '0' + hour : hour}:{minutes}</text>,
        <text x={i * 5 + 3.5} y={svgHeight - 4} style={textStyle}>{day}</text>,
        <text x={i * 5 + 3.1} y={svgHeight - 2} style={textStyle}>{month} {index}</text>])
      } else {
        ...
        <text x={i * 5 + 3.2} y={svgHeight - 6} style={textStyle}>{hour2 < 12 ? '0' + hour2 : hour2}:{minutes}</text>,
        <text x={i * 5 + 3.5} y={svgHeight - 4} style={textStyle}>{day}</text>,
        <text x={i * 5 + 3.3} y={svgHeight - 2} style={textStyle}>{month} {index}</text>])
      }
    }
    return minorYAxis
  }
